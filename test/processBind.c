...
void bindByPid(DWORD pid, int target_mode, const WCHAR *target_exe)
{
    ...
    if (!hProcess)
    {
        wprintf(L"无法打开 PID=%lu，错误: %ls\n", pid, get_last_error_message());
        return;
    }
    if (SetProcessAffinityMask(hProcess, mask))
    {
        wprintf(L"绑定成功：PID=%lu, %s -> %ls\n", pid, target_exe, core_label);
    }
    else
    {
        wprintf(L"绑定失败：PID=%lu, %s，错误代码: %lu\n", pid, target_exe, GetLastError());
    }
    ...
    if (hProcess)
    {
        ...
        if (QueryFullProcessImageNameW(hProcess, 0, exe_path, &len))
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    bindByPid(pid, mode[i], exe_name[i]);
                }
            }
        }
        CloseHandle(hProcess);
...
void scan_and_bind_all()
{
    for (int i = 0; i < count; i++)
    {
        DWORD pid = find_process_id(exe_name[i]);
        if (pid == 0)
            continue;;
        bindByPid(pid, mode[i], exe_name[i]);
    }
